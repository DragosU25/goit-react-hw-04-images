...
export default function App() {
  ...
  const handleSearchSubmit = async searchQuery => {
    ...
    try {
      const response = await axios.get(
        `https://pixabay.com/api/?key=${API_KEY}&q=${searchQuery}&image_type=photo&orientation=horizontal&per_page=${INITIAL_PER_PAGE}`
      );
  ...
  const loadMoreImages = async event => {
    ...
    setLoading(true);
    try {
      const nextPage = Math.ceil(images.length / INITIAL_PER_PAGE) + 1;
      const response = await axios.get(
